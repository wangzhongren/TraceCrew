    ...
    <div className="flex gap-3 pb-3">
      ...
      <div className="flex-1 min-w-0 rounded-lg border-l-2 overflow-hidden" style={{ background: '#0d1117', borderColor: '#21262d', borderLeftColor: color }}>
        <div className="px-3 py-1.5 border-b flex items-center gap-2" style={{ borderColor: '#21262d' }}>
          <span className="text-[10px] font-bold tracking-wider px-1.5 py-0.5 rounded" style={{ background: color + '20', color }}>PLAN</span>
          <span className="text-[10px]" style={{ color: '#8b949e' }}>分析报告</span>
        </div>
        <div className="px-3 py-2 overflow-x-auto">
          <ReactMarkdown remarkPlugins={[remarkGfm]}
            components={{
              ...
              h4: ({ children }: any) => <h4 className="text-[11px] font-medium mt-2 mb-1" style={{ color: '#e6e6e6' }}>{children}</h4>,
              ...
              table: ({ children }: any) => <div className="overflow-x-auto my-2"><table className="w-full text-[10px] border-separate border-spacing-0">{children}</table></div>,
              th: ({ children }: any) => <th className="border border-[#21262d] bg-[#161b22] px-2 py-1 text-[10px] font-medium" style={{ color: '#e6e6e6' }}>{children}</th>,
              td: ({ children }: any) => <td className="border border-[#21262d] px-2 py-1 text-[10px]" style={{ color: '#c9d1d9' }}>{children}</td>,
